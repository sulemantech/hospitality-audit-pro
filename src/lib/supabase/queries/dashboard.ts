...
export async function getDashboardMetrics() {
  ...
  const [complaintsRes, reviewsRes, alertsRes, actionItemsRes] = await Promise.all([
    supabase.from("complaints").select("status, severity, created_at"),
    supabase.from("reviews").select("rating, sentiment, review_date"),
    supabase.from("alerts").select("severity, status"),
    supabase.from("action_items").select("status, due_date, priority"),
  ]);
...
export async function getPropertyHealth() {
  const supabase = await createClient();
  const { data, error } = await supabase.from("property_health").select("*");
...
export async function getActiveAlerts(limit = 10) {
  ...
  const { data, error } = await supabase
    ...
    .eq("status", "active")
    .order("severity", { ascending: true }) // critical sorts first alphabetically — handled in component
...
export async function getTopActionItems(limit = 5) {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("action_items")
